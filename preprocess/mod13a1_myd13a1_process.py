...
def create_rasters(frame, productType):
    """
    Description:
    ...
    for index, row in frame.iterrows():
        ...
        for name in row["image"]:
            # Construct the paths for both EVI and NDVI raster files.
            eviPath = f'HDF4_EOS:EOS_GRID:"{productFolder}/{name}":MODIS_Grid_16DAY_500m_VI:"500m 16 days EVI"'
            eviPaths.append(eviPath)
            ndviPath = f'HDF4_EOS:EOS_GRID:"{productFolder}/{name}":MODIS_Grid_16DAY_500m_VI:"500m 16 days NDVI"'
            ndviPaths.append(ndviPath)

        # Apply mosaic and clip operations to the raster files.
        mosaic_and_clip_rasters(eviPaths, row["date"], productType, "evi")
        mosaic_and_clip_rasters(ndviPaths, row["date"], productType, "ndvi")


def mosaic_and_clip_rasters(paths, date, productType, mode=""):
    """
    Description:
    ...
    Arguments:
        ...
        productType (str): The type of the product.
        mode (str, optional): Mode of processing. Defaults to "".
    ...
    intermediateName = f"{path}/{date}_{mode}_merge.tif"
    outputTifName = f"{path}/{date}_{mode}_warp.tif"
    outputNetName = f"{path}/{date}_{mode}_warp.nc"
...
def main(args):
    ...
    print(f"Executing `mod13a1_myd13a1_process.py` with arguments: `{args[0]}`, `{args[1]}`.")
    ...
    for productType in productTypes:
        ...
        create_rasters(groupedFrame, productType)
