...
def make_temps(dataset, path, variable, bandName):
    ds = xr.open_dataset(os.path.join(path, dataset), engine="netcdf4")
    ds = ds.drop_vars('crs')
    ...
    newVariable_temp = xr.concat([selectedVars_temp[varName] for varName in selectedVars_temp.data_vars], dim=list(timeDict_temp.values()))
...
def main():
    ...
    convert_grb2_to_nc(inputMeteoFolder, intermediateMeteoFolder)

    # Get a list of netCDF files in the specified directory.
    files = [file for file in os.listdir(intermediateMeteoFolder) if file.endswith(".nc")]
    #files = ['WRF-{}.nc'.format(date)]

    for file in files:
        date = file.split("-")[1].split(".")[0]
        outputFilePath = os.path.join(outputMeteoFolder, date + ".nc")
        finalFilePath = os.path.join(finalMeteoFolder, date + "_meteo.nc")
        ...
        if not os.path.exists(outputFilePath):
            ...
            # u-component
            datasetU = concatenate_variables(file, intermediateMeteoFolder,
                                             variable="u-component of wind [m/s]", bandName="u10")
            # v-component
            datasetV = concatenate_variables(file, intermediateMeteoFolder,
            ...
            dataset_temp = make_temps(file, intermediateMeteoFolder,'Temperature [C]', "temp")
            dataset_dew = make_temps(file, intermediateMeteoFolder,'Dew point temperature [C]' ,'dew_temp')
            dataset_tp = make_temps(file, intermediateMeteoFolder,'Total precipitation [kg/(m^2)]','tp')
            merged = xr.merge([ds,dataset_temp,dataset_dew,dataset_tp])
            merged = merged.assign_coords({"time": date})
        ...
        if not os.path.exists(finalFilePath):
            ...
            while start_date <= end_date:
                ...
                if os.path.isfile(os.path.join(outputMeteoFolder,filename)):
                # Search for files with the generated filename
                    pr_files.append(os.path.join(outputMeteoFolder, filename))
                start_date = start_date + timedelta(days=1)
            if len(pr_files)>1:
                # Open the files using mfdataset
